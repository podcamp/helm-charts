...
externalPostgresql:
  ...
  existingSecretUsernameKey: "my-postgres-username-key"
...
externalRedis:
  ...
  existingSecretPasswordKey: "my-redis-password-key"
```
...
taskRunners:
  mode: external
  image:
    repository: "n8nio/n8n-task-runner"
    tag: "2.0.0"
    pullPolicy: Always
```
...
| externalPostgresql | object | `{"database":"n8n","existingSecret":"","existingSecretUsernameKey":"postgres-username","existingSecretPasswordKey":"postgres-password","host":"","password":"","port":5432,"username":"postgres"}` | External PostgreSQL parameters |
...
| externalPostgresql.existingSecretUsernameKey | string | `"postgres-username"` | The key in `externalPostgresql.existingSecret` that stores the PostgreSQL username. |
...
| externalRedis | object | `{"clusterNodes":[],"database":0,"dualStack":false,"existingSecretPasswordKey":"redis-password","existingSecret":"","existingSecretUsernameKey":"redis-username","host":"","password":"","port":6379,"tls":{"enabled":false},"username":""}` | External Redis parameters |
...
| externalRedis.existingSecretUsernameKey | string | `"redis-username"` | The key in `externalRedis.existingSecret` that stores the Redis username. |
| externalRedis.existingSecretPasswordKey | string | `"redis-password"` | The key in `externalRedis.existingSecret` that stores the Redis password. |
...
| taskRunners.image | object | `{"pullPolicy":"","repository":"","tag":""}` | Override the task runner container image. Defaults to the main image values when left empty. |
| taskRunners.image.pullPolicy | string | `""` | Task runner image pull policy override |
| taskRunners.image.repository | string | `""` | Task runner image repository override |
| taskRunners.image.tag | string | `""` | Task runner image tag override |
